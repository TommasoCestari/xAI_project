...
def main():
    parser = argparse.ArgumentParser(description="Compute Relative Metrics from saved IG run")
    ...
    for i in range(0, n_samples, batch_size):
        ...
        batch_attr = ig_attrs[i:end]
        batch_preds = ig_preds[i:end]
    ...
    print("\n" + "="*50)
    print("RISULTATI FINALI INTEGRATED GRADIENTS (Mean Confidence Drop):")
    print("="*50)
    print(f"Ablation Top-20% Pixel   : {all_drop_top.mean():.4f}")
    print(f"Ablation Random-20% Pixel: {all_drop_rand.mean():.4f}")
    print(f"VANTAGGIO IG (Top-Rand)  : {relative_drops.mean():.4f}")
    print("="*50)
